 ...
 */
@Getter
public class WareneingangEvent4 extends BusinessObjectEvent {
    ...
    private static final Predicate<EpcisEvent> PREDICATE = isEventType(EventType.OBJECT)
            ...
            .and(EventPredicates.hasBizStep(BizStep.RECEIVING))
            .and(EventPredicates.hasDisposition(Disposition.IN_PROGRESS))
    ...
    public WareneingangEvent4(EpcisEvent event) {
    ...
    public static IntellipackBusinessEventFactory<ProduktionEvent1> getFactory() {
        if (FACTORY == null){
            ...
            validator.addRule(new ValidationRule(EventPredicates.hasBizStep(BizStep.RECEIVING), "Event bizStep should be RECEIVING! "));
            validator.addRule(new ValidationRule(EventPredicates.hasDisposition(Disposition.IN_PROGRESS), "Event disposition should be IN_PROGRESS! "));
