 ...
 */
@Getter
public class WarenausgangEvent3 extends BusinessObjectEvent {
    ...
    private static final Predicate<EpcisEvent> PREDICATE = isEventType(EventType.OBJECT)
            ...
            .and(EventPredicates.hasBizStep(BizStep.SHIPPING))
            .and(EventPredicates.hasDisposition(Disposition.IN_TRANSIT))
    ...
    public WarenausgangEvent3(EpcisEvent event) {
    ...
    public static IntellipackBusinessEventFactory<ProduktionEvent1> getFactory() {
        if (FACTORY == null){
            ...
            validator.addRule(new ValidationRule(EventPredicates.hasBizStep(BizStep.SHIPPING), "Event bizStep should be SHIPPING! "));
            validator.addRule(new ValidationRule(EventPredicates.hasDisposition(Disposition.IN_TRANSIT), "Event disposition should be IN_TRANSIT! "));
